describe('School Free self-service onboarding', () => {
  it('takes a new representative from public discovery to first-run guidance', () => {
    ...
    cy.intercept('GET', '**/api/organizations/new-school/entitlements', { id: 'entitlement', organization_id: 'new-school', plan_key: 'school_free', status: 'active', source: 'system', effective_from: '2026-01-01T00:00:00Z', effective_until: null, capabilities: ['school_roster_import', 'school_persistent_teams', 'school_match_playing_xi'], excluded_capabilities: [], created_at: '2026-01-01T00:00:00Z', updated_at: '2026-01-01T00:00:00Z' })
    ...
    cy.location('pathname').should('eq', '/schools/new-school'); cy.contains('Start your School workspace').should('be.visible'); cy.contains('Upload Players').should('be.visible'); cy.contains('Create Teams').should('be.visible'); cy.contains('Assign Players').should('be.visible'); cy.contains('Create First Match').should('be.visible')
  })
})
