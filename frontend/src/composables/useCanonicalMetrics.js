...
export function useCanonicalMetrics(gameIdInput) {
    ...
    async function refresh() {
        if (!gameId.value || isRefreshing.value)
            return;
        try {
            isRefreshing.value = true;
            lastError.value = null;
            const game = await apiRequest(`/games/${gameId.value}`);
 ...
 */
export function formatRunRate(rate, decimals = 2) {
 ...
 */
export function formatBallsAsOvers(balls) {
