...
function crossed(prevVal, nextVal, mark) {
    return prevVal < mark && nextVal >= mark;
}
function detectMilestones(prevB, nextB) {
    if (!nextB)
        return [];
...
export function diffToEvents(prevSnap, nextSnap) {
    ...
    if (lb) {
        ...
        if (lb.isWicket) {
            ...
            const dismissedWasDuck = (dismissedId && ((prevStriker?.id === dismissedId && safe(prevStriker?.runs) === 0) ||
 ...
 */
export function useHighlights(enable, durationMs) {
    const queue = ref([]);
    ...
    function enqueue(events) {
        ...
        if (!isPlaying.value && enable.value)
            playNext();
    }
    function enqueueFromSnapshots(prev, next) {
        if (!enable.value)
            return;
