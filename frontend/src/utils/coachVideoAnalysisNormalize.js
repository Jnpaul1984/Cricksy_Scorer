...
function normalizeRateTo01(value) {
...
function coerceMetricValueTo01(value) {
...
function extractTopFindings(findingsRaw, reportRaw) {
    const findings = [];
    const report = pickFirstObject(reportRaw);
    const reportIssues = Array.isArray(report?.key_issues) ? report?.key_issues : [];
    const reportDrills = Array.isArray(report?.drills) ? report?.drills : [];
    const drillNames = reportDrills
    ...
    if (strengthAreas.length > 0) {
        findings.push({
            ...
            whyItMatters: 'Why it matters: Reinforcing strengths makes your base more reliable under pressure.',
...
function buildTakeaways(numbers, findings, reportRaw) {
    const takeaways = [];
    const report = pickFirstObject(reportRaw);
...
function overallLevelFromMetrics(metrics) {
...
export function normalizeCoachVideoAnalysis(job) {
    ...
    const isInProgressStatus = status === 'queued' ||
        ...
        status === 'quick_done' ||
        status === 'deep_running';
    const bestResults = (job?.deep_results ?? job?.quick_results ?? job?.results ?? null);
    ...
    const totalFrames = pickFirstNumber(pose?.total_frames, poseSummaryNested?.frame_count, poseSummaryLegacy?.total_frames);
    const sampledFrames = pickFirstNumber(metricsSummary?.total_frames, pose?.sampled_frames, poseSummaryLegacy?.sampled_frames);
    const detectedFrames = pickFirstNumber(pose?.detected_frames, poseSummaryLegacy?.frames_with_pose);
    const detectionRateRaw = pickFirstNumber(
    // Newer pose schema: pose.metrics.detection_rate is percent (0..100)
    pose?.metrics?.detection_rate, pose?.pose_summary?.detection_rate, pose?.detection_rate_percent, pose?.detection_rate, poseSummaryLegacy?.detection_rate_percent);
    let detectionRate = null;
    ...
    const reportRaw = (resultsObj?.['report'] ?? coachObj?.['report'] ?? null);
