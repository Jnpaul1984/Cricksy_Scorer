...
export function fmtSR(runs, balls) {
...
export function oversNotationToFloat(ov) {
    if (ov == null)
        return 0;
...
export function fmtEconomy(runsConceded, ballsBowled) {
...
export function oversDisplayFromBalls(balls) {
    const b = Math.max(0, Math.trunc(Number(balls) || 0));
...
export function isLegalBall(d) {
    // If your API provides d.legal, prefer that.
    if ('legal' in d)
        return Boolean(d.legal);
    // Otherwise, wides/no-balls do NOT increment balls; byes/leg-byes DO.
    const t = String(d.extra_type ?? d.extra ?? '');
    if (!t)
        return true; // no extra => legal ball
    return t === 'b' || t === 'lb';
}
export function ballsFromOversNotation(ov) {
    if (ov == null)
        return 0;
    const n = typeof ov === 'number' ? ov : Number(ov);
    if (!Number.isFinite(n))
...
export function accumulateBowling(deliveries, bowlerId) {
    let runs = 0, balls = 0;
    const pid = String(bowlerId);
    for (const d of deliveries) {
        if (String(d.bowler_id || '') !== pid)
            continue;
...
export function deriveBowlerFigures(deliveries, bowlerId) {
    let runs = 0;
    let balls = 0;
    let maidens = 0;
    const pid = String(bowlerId);
    ...
    const overNum = (d) => {
        ...
        if (typeof o === 'number')
            return Math.floor(o);
        if (typeof o === 'string')
            return Number(o.split('.')[0] || 0) || 0;
        return 0;
    };
    const extraCode = (d) => String(d.extra ?? d?.extra_type ?? '');
    for (const d of deliveries) {
        if (String(d.bowler_id || '') !== pid)
            continue;
        const x = extraCode(d);
    ...
    for (const key of Object.keys(overRuns)) {
        ...
        const legalBallsInOver = deliveries.filter(d => String(d.bowler_id || '') === pid &&
            overNum(d) === over &&
            (extraCode(d) !== 'wd' && extraCode(d) !== 'nb')).length;
