...
import { createVideoSession, listVideoSessions, initiateVideoUpload, uploadToPresignedUrl, completeVideoUpload, getAnalysisJobStatus, listAnalysisJobs, ApiError, } from '@/services/coachPlusVideoService';
export const useCoachPlusVideoStore = defineStore('coachPlusVideo', () => {
    ...
    // Job polling state
    const pollingJobs = ref(new Map());
    const jobStatusMap = ref(new Map());
    const lastFetchedAt = ref(new Map()); // Timestamp when job was last fetched
    // ============================================================================
    // COMPUTED
    // ============================================================================
    ...
    const processingJobs = computed(() => allJobs.value.filter((j) => isJobInProgress(j)));
    /**
     * Check if a job's cached data is stale (older than 30 seconds)
     */
    function isJobStale(jobId) {
     ...
     */
    async function startUpload(file, sessionId, sampleFps = 10, includeFrames = false, analysisMode = null) {
        error.value = null;
        // Validate file
     ...
     */
    function pollJobUntilComplete(jobId, intervalMs = 5000) {
    ...
    async function updateJobStatus(jobId) {
        try {
            ...
            if (existing && isJobTerminal(existing) && !isJobTerminal(job)) {
                console.warn(`[coachPlusVideo] Prevented terminal status override: ${existing.status} -> ${job.status} for job ${jobId}`);
     ...
     */
    async function forceFetchJob(jobId) {
        try {
            const job = await getAnalysisJobStatus(jobId);
            jobStatusMap.value.set(jobId, job);
            lastFetchedAt.value.set(jobId, Date.now());
