...
const isSuperuser = (state) => state.user?.role === 'superuser' || Boolean(state.user?.is_superuser);
...
export const useAuthStore = defineStore('auth', {
    ...
    getters: {
        ...
        is: (state) => (role) => (state.user?.role ?? null) === role,
        ...
        isSuperuser,
        canScore: (state) => isSuperuser(state) || state.user?.role === 'coach_pro' || state.user?.role === 'coach_pro_plus' || state.user?.role === 'org_pro',
        canAnalyze: (state) => isSuperuser(state) || state.user?.role === 'analyst_pro' || state.user?.role === 'org_pro',
        canManageTournaments: (state) => isSuperuser(state) || state.user?.role === 'org_pro',
        ...
        createdAt: (state) => state.user?.created_at || null,
        orgId: (state) => state.user?.org_id || null,
        subscription: (state) => state.user?.subscription || null,
        planName: (state) => state.user?.subscription?.plan || state.user?.role || 'free',
        requiresPasswordChange: (state) => state.user?.requires_password_change || false,
    },
    actions: {
        hasAnyRole(roles) {
