...
export function validateEnv(options) {
    ...
    if (config.enableAiFeatures || options?.requireLLMKeys) {
        if (!config.geminiApiKey) {
            warnings.push("VITE_GEMINI_API_KEY not set - Gemini provider will be unavailable");
        }
        if (!config.anthropicApiKey) {
            warnings.push("VITE_ANTHROPIC_API_KEY not set - Claude fallback will be unavailable");
        }
...
export function isAiAvailable() {
    const config = env();
    return (config.enableAiFeatures &&
