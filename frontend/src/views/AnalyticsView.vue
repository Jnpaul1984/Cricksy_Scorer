...
const manhattanChart = computed(() => {
  ...
  if (!len) return { labels: [] as string[], series: [] as Array<{ label: string; data: (number | null)[] }> }
...
const req = computed(() => {
  ...
  if (target == null) return { rrr: null as number | null, remainingOvers: null as number | null, remainingRuns: null as number | null }
  const remainingRuns = Math.max(0, Number(target) - currRuns.value)
  const remainingBalls = Math.max(0, Number(snapshot.value?.overs_limit || 0) * 6 - ballsBowled.value)
...
const extrasTotals = computed<Record<string, number>>(() => snapshot.value?.extras_totals || {})
const fallOfWickets = computed(() => (snapshot.value?.fall_of_wickets || []) as Array<{ score_at_fall?: number; over?: number; batter_id?: string }>)
...
const phaseSplits = computed(() => {
  ...
  if (!oversLimit) return [] as Array<{ name: string; runs: number; overs: number; wkts: number }>
    ...
    <section v-if="selectedId && snapshot" class="grid">
      <div class="card">
        ...
        <div v-if="req.rrr != null">
          Required: {{ (req.rrr as number).toFixed(2) }} rpo ({{ req.remainingRuns }} runs in {{ req.remainingOvers?.toFixed(1) }} overs)
        </div>
      </div>
