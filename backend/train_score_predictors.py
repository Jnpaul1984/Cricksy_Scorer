...
def _rotation_ratio(window: pd.Series) -> float:
    singles = window[(window >= 1) & (window <= 2)].sum()
    total = window.sum()
    if total <= 0:
        return 0.0
    return float(singles) / float(total)
...
def engineer_features(df: pd.DataFrame, match_format: str) -> pd.DataFrame:
    ...
    def calc_rolling_features(group):
        # Last 5 runs
        span = max(1, min(5, len(group)))
        group["last_5_runs"] = group["total_runs"].diff(periods=span).fillna(0)

        rolling_runs = group["total_runs"].diff().fillna(0)

        # Dot ratio last over (simplified)
        group["dot_ratio_last_over"] = (rolling_runs == 0).rolling(6, min_periods=1).mean()

        # Strike rotation (runs scored in singles/twos)
        group["strike_rotation_last_6"] = rolling_runs.rolling(6, min_periods=1).apply(
            _rotation_ratio,
            raw=False,
        )

        # Run rate last 5 overs
        group["run_rate_last_5"] = rolling_runs.rolling(span, min_periods=1).mean()

        # Boundaries ratio
        group["boundary_ratio"] = (rolling_runs >= 4).rolling(10, min_periods=1).mean()
...
def train_score_predictor(match_format: Literal["t20", "odi"]) -> None:
    ...
    print("-" * 30)
    for actual, pred in list(zip(y_test, y_pred, strict=True))[:10]:
