...
def engineer_features(df: pd.DataFrame, match_format: str) -> pd.DataFrame:
    ...
    if match_format == 't20':
        df['match_phase_id'] = pd.cut(
            df['completed_overs'], 
            bins=[0, 6, 15, 20], 
            labels=[0, 1, 2],
            include_lowest=True
        ).cat.codes.fillna(0).astype(int)
    else:  # ODI
        df['match_phase_id'] = pd.cut(
            df['completed_overs'], 
            bins=[0, 10, 40, 50], 
    ...
    def calc_rolling_features(group):
        ...
        group['dot_ratio_last_over'] = (group['total_runs'].diff().fillna(0) == 0).rolling(6, min_periods=1).mean()
        
        # Strike rotation (runs scored in singles/twos)
        group['strike_rotation_last_6'] = group['total_runs'].diff().fillna(0).rolling(6, min_periods=1).apply(
            lambda x: (x[x.between(1, 2)].sum() / x.sum() if x.sum() > 0 else 0)
        )
        
        # Run rate last 5 overs
        group['run_rate_last_5'] = group['total_runs'].diff(periods=min(5, len(group))).fillna(0) / 5
        
        # Boundaries ratio
        group['boundary_ratio'] = (group['total_runs'].diff().fillna(0) >= 4).rolling(10, min_periods=1).mean()
