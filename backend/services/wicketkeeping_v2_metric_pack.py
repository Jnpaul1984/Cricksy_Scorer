...
def attach_wicketkeeping_v2_metric_pack(
    *,
    ...
    if (discipline or "").strip().lower() != "wicketkeeping":
    ...
    built_results = _build_wicketkeeping_metric_results(
    ...
    retained = [
        ...
        if not (
            ...
            and item.get("metric_id") in _WICKETKEEPING_V2_METRIC_IDS
        )
    ]
    ...
    if isinstance(results_payload["meta"], dict):
        results_payload["meta"]["wicketkeeping_v2_metric_pack"] = _build_metric_pack_summary(
            built_results,
            repetitions,
        )

    insights = build_wicketkeeping_v2_findings_insights(built_results)
    findings_payload = results_payload.get("findings")
    if isinstance(findings_payload, dict):
        findings_payload["wicketkeeping_v2"] = insights
    report_payload = results_payload.get("report")
    if isinstance(report_payload, dict):
        report_payload["wicketkeeping_v2"] = insights

    return results_payload


def build_wicketkeeping_v2_findings_insights(
    ...
    for metric in metric_results:
        label = metric.metric_id.replace("wicketkeeping_", "").replace("_", " ")
        if (
...
def _build_wicketkeeping_metric_results(
    *,
    ...
    capture = capture_profile or CaptureProfile(
        ...
        metric_version=_WICKETKEEPING_V2_METRIC_VERSION,
        source_model=source_model,
    )
    ...
    return [
        ...
        _build_metric_result(
            ...
            samples=_single_phase_metric(
                ...
                phase_lookup=phase_lookup,
                frames=frames,
                phase_names=("movement",),
                measure_fn=_measure_lateral_displacement_ratio,
            ),
            sample_fps=sample_fps,
            source_video_fps=source_video_fps,
            camera_view=camera_view,
            ...
            valid_range=(0.0, 4.0),
            classification_fn=lambda value: (
                "STRONG" if value <= 1.2 else "DEVELOPING" if value <= 2.0 else "NEEDS_ATTENTION"
            ),
            limitations=[
        ...
        _build_metric_result(
            ...
            valid_range=(0.0, 1.8),
            classification_fn=lambda value: (
                "STRONG" if value <= 0.22 else "DEVELOPING" if value <= 0.34 else "NEEDS_ATTENTION"
            ),
            limitations=[
        ...
        _build_metric_result(
            ...
            phase="set",
            unit="ratio",
            samples=_standing_context_delta_samples(
                repetitions=repetitions,
                phase_lookup=phase_lookup,
                frames=frames,
            ...
            valid_range=(0.0, 1.8),
            classification_fn=lambda value: (
                "STRONG" if value <= 0.2 else "DEVELOPING" if value <= 0.35 else "NEEDS_ATTENTION"
            ),
            limitations=[
        ...
        _build_metric_result(
            metric_id="wicketkeeping_leg_side_movement_lateral_displacement_ratio",
            phase="movement",
            unit="ratio",
            samples=_single_phase_metric(
                repetitions=repetitions,
                ...
                phase_names=("movement",),
                measure_fn=_measure_lateral_displacement_ratio,
                repetition_filter=lambda rep: bool(
                    subtype_map.get(rep.repetition_id, {}).get("leg_side")
                ),
            ),
            sample_fps=sample_fps,
            source_video_fps=source_video_fps,
            ...
            valid_range=(0.0, 4.0),
            classification_fn=lambda value: (
                "STRONG" if value <= 1.35 else "DEVELOPING" if value <= 2.25 else "NEEDS_ATTENTION"
            ),
            limitations=[
        ...
        _build_metric_result(
            metric_id="wicketkeeping_stumping_action_wrist_compactness_ratio",
            phase="action",
            unit="ratio",
            samples=_single_phase_metric(
                repetitions=repetitions,
                ...
                repetition_filter=lambda rep: bool(
                    subtype_map.get(rep.repetition_id, {}).get("stumping")
                ),
                confidence_adjuster=lambda rep, phase: (
                    0.1
                    if _phase_has_object_hint(phase, {"stumps", "ball_tracking", "collection"})
                    else -0.08
                ),
            ),
            sample_fps=sample_fps,
            ...
            valid_range=(0.0, 2.5),
            classification_fn=lambda value: (
                "STRONG" if value <= 0.95 else "DEVELOPING" if value <= 1.25 else "NEEDS_ATTENTION"
            ),
            limitations=[
...
def _build_metric_result(
    *,
    ...
    if (
        ...
        and validity_state == ValidityState.NOT_MEASURABLE
    ):
    ...
    *,
    ...
    for repetition in repetitions:
        ...
        if confidence_adjuster is not None:
            confidence = round(
                max(0.0, min(1.0, confidence + confidence_adjuster(repetition, phase))), 3
            )
        samples.append(
            _MetricSample(
...
def _standing_context_delta_samples(
    *,
    repetitions: list[RepetitionActionRecordV2],
    phase_lookup: dict[tuple[str, str], PhaseRecordV2],
    frames: list[_NormalizedFrame],
    ...
    up_samples = _single_phase_metric(
        repetitions=repetitions,
        phase_lookup=phase_lookup,
        frames=frames,
        phase_names=("set",),
        measure_fn=_measure_set_depth_ratio,
        repetition_filter=lambda rep: (
            subtype_map.get(rep.repetition_id, {}).get("standing") == "up"
        ),
    )
    back_samples = _single_phase_metric(
        repetitions=repetitions,
        phase_lookup=phase_lookup,
        frames=frames,
        phase_names=("set",),
        measure_fn=_measure_set_depth_ratio,
        repetition_filter=lambda rep: (
            subtype_map.get(rep.repetition_id, {}).get("standing") == "back"
        ),
    )
    if not up_samples or not back_samples:
        return []

    up_value = mean([sample.value for sample in up_samples])
    back_value = mean([sample.value for sample in back_samples])
    source_sample = up_samples[0]
    return [
        _MetricSample(
            repetition_id=source_sample.repetition_id,
            phase_id=source_sample.phase_id,
            value=abs(up_value - back_value),
            confidence=round(
                max(
                    0.0,
                    min(
                        1.0,
                        mean([sample.confidence for sample in up_samples + back_samples]) - 0.03,
                    ),
                ),
                3,
            ),
            visibility=mean([sample.visibility for sample in up_samples + back_samples]),
            start_ts=source_sample.start_ts,
            end_ts=source_sample.end_ts,
            start_frame=source_sample.start_frame,
            end_frame=source_sample.end_frame,
            evidence_refs=[
                EvidenceRef(ref_type="context_pair", ref_id="standing_up_vs_back"),
            ],
        )
    ]
...
def _measure_lateral_displacement_ratio(
    ...
    if len(nose_x) < 2 or not shoulder_widths:
        return (None, 0.0)
    displacements.append(max(nose_x) - min(nose_x))
    value = displacements[0] / mean(shoulder_widths)
    return (value, _visibility(window, displacements))
...
def _measure_set_depth_ratio(window: list[_NormalizedFrame]) -> tuple[float | None, float]:
    values: list[float] = []
    for frame in window:
        if not frame.detected:
            continue
        ...
        left_ankle = frame.keypoints.get("left_ankle")
        right_ankle = frame.keypoints.get("right_ankle")
        if not (left_hip and right_hip and left_knee and right_knee and left_ankle and right_ankle):
            continue
        knee_mid = ((left_knee[0] + right_knee[0]) / 2.0, (left_knee[1] + right_knee[1]) / 2.0)
        hip_mid = ((left_hip[0] + right_hip[0]) / 2.0, (left_hip[1] + right_hip[1]) / 2.0)
        ankle_mid = ((left_ankle[0] + right_ankle[0]) / 2.0, (left_ankle[1] + right_ankle[1]) / 2.0)
        shin = _distance(knee_mid, ankle_mid)
        if shin <= 0.001:
            continue
        values.append(_distance(hip_mid, knee_mid) / shin)
    return (_mean_or_none(values), _visibility(window, values))
...
def _build_metric_pack_summary(
    metric_results: list[CoachingMetricResultV2], repetitions: list[RepetitionActionRecordV2]
) -> dict[str, Any]:
    valid_count = sum(
        1
        for metric in metric_results
    ...
    subtype_counts = _summarize_repetition_subtypes(repetitions)
    return {
        "enabled": True,
        "version": _WICKETKEEPING_V2_METRIC_VERSION,
        ...
        "cost_classification": {
            ...
            "context_metrics": "low_cost_cpu",
            "stumping_leg_side_context": "ball_or_object_evidence_dependent",
        },
    }


def _infer_repetition_subtypes(
    repetitions: list[RepetitionActionRecordV2], phase_lookup: dict[tuple[str, str], PhaseRecordV2]
) -> dict[str, dict[str, Any]]:
    payload: dict[str, dict[str, Any]] = {}
    for repetition in repetitions:
        text_parts = [str(repetition.action_type or "")]
        for phase_name in ("set", "movement", "collection", "action"):
            phase = phase_lookup.get((repetition.repetition_id, phase_name))
            if phase is None:
                continue
            text_parts.extend(
                [str(ref.ref_id or "") for ref in phase.evidence_refs if ref.ref_id is not None]
            )
            text_parts.extend(
                [str(ref.label or "") for ref in phase.evidence_refs if ref.label is not None]
            )
        combined = " ".join(text_parts).lower()
        payload[repetition.repetition_id] = {
            "standing": (
                "up"
                if "standing_up" in combined or "stand_up" in combined
                else "back"
                if "standing_back" in combined or "stand_back" in combined
                else "unknown"
            ),
            "leg_side": "leg_side" in combined or "leg-side" in combined,
            "stumping": "stump" in combined,
        }
    return payload


def _summarize_repetition_subtypes(repetitions: list[RepetitionActionRecordV2]) -> dict[str, int]:
    counts = {
        "standing_up": 0,
        "standing_back": 0,
        "leg_side": 0,
    ...
    for repetition in repetitions:
        action_type = str(repetition.action_type or "").lower()
        matched = False
        if "standing_up" in action_type:
            counts["standing_up"] += 1
            matched = True
        if "standing_back" in action_type:
            counts["standing_back"] += 1
            matched = True
        if "leg_side" in action_type or "leg-side" in action_type:
            counts["leg_side"] += 1
            matched = True
        if "stump" in action_type:
            counts["stumping"] += 1
            matched = True
        if not matched:
            counts["generic"] += 1
...
def _phase_has_object_hint(phase: PhaseRecordV2, hints: set[str]) -> bool:
    labels = [str(ref.label or "").lower() for ref in phase.evidence_refs]
    ids = [str(ref.ref_id or "").lower() for ref in phase.evidence_refs]
    types = [str(ref.ref_type or "").lower() for ref in phase.evidence_refs]
    combined = " ".join(labels + ids + types)
...
def _angle_degrees(
    ...
    if magnitude <= 1e-9:
        return 180.0
    cosine = max(
        -1.0, min(1.0, ((vector_ab[0] * vector_cb[0]) + (vector_ab[1] * vector_cb[1])) / magnitude)
    )
