...
async def check_agent_limits(db, agentKey, userId, tokensRequested):
    ...
    # Count runs for this agent/user today (parameterized)
    runs_today = await db.execute(
        "SELECT COUNT(*), COALESCE(SUM(tokensOut),0) FROM agent_runs "
    ...
    if tokensRequested > AGENT_BUDGETS[agentKey]["max_tokens_per_run"]:
        return False, "Token limit per run exceeded"
    # Check global daily tokens
    total_tokens_today = await db.execute(
        "SELECT COALESCE(SUM(tokensOut),0) FROM agent_runs WHERE createdAt >= :start",
        {"start": today},
    )
...
async def get_recent_runs(db, limit=10):
    return (
        await db.execute(
            "SELECT * FROM agent_runs ORDER BY createdAt DESC LIMIT :limit",
            {"limit": limit},
        )
    ).fetchall()
