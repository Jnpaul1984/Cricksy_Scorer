...
class TacticalSuggestionEngine:
    ...
    EFFECTIVENESS_THRESHOLD = 40  # Minimum effectiveness score to recommend (adjusted for realistic data)
    
    @staticmethod
    def get_best_bowler(
        bowlers: list[dict],
        batter_profile: dict,
        recent_bowlers: list[str] = None,
    ) -> Optional[BestBowlerSuggestion]:
        """
        ...
        for bowler_data in bowlers:
            ...
            weakness_match = TacticalSuggestionEngine._match_bowler_to_weakness(
                bowler, batter
            )
    ...
    def analyze_weakness(
        batter_profile: dict,
        dismissal_history: list[dict] = None,
    ) -> WeaknessAnalysis:
        """
    ...
    def recommend_fielding(
        ...
        for idx, zone in enumerate(priority_zones[:3], 1):
            ...
            recommended_positions.append(FieldingZone(
                position=position,
                priority=idx,
        ...
        return FieldingSetup(
            ...
            primary_zone=f"{priority_zones[0].line}-{priority_zones[0].length}" if priority_zones else "off-good",
    ...
    def _match_bowler_to_weakness(
        ...
        if bowler.common_delivery == DeliveryType.PACE and batter.pace_weakness > 50:
            score += 25
        elif bowler.common_delivery == DeliveryType.SPIN and batter.spin_weakness > 50:
            score += 25
