...
async def calculate_org_stats(db: AsyncSession, org_id: str) -> dict:
    ...
    stmt = select(models.Game).where(
        (models.Game.team_a_id.in_(team_ids)) | (models.Game.team_b_id.in_(team_ids))
    )
    result = await db.execute(stmt)
    ...
    for game in games:
        ...
        stmt = select(models.Delivery).where(models.Delivery.game_id == game.id)
        result = await db.execute(stmt)
...
async def get_org_teams_stats(db: AsyncSession, org_id: str) -> list[dict]:
    ...
    for team in org_teams:
        ...
        stmt = select(models.Game).where(
            (models.Game.team_a_id == team.id) | (models.Game.team_b_id == team.id)
        )
        result = await db.execute(stmt)
        games = result.scalars().all()
        ...
        for game in games:
            ...
            state = game.current_state or {}
            is_batting_team = (
                game.batting_team_id == team.id if hasattr(game, "batting_team_id") else False
            )
