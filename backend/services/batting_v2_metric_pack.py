...
def attach_batting_v2_metric_pack(
    *,
    ...
    if (discipline or "").strip().lower() != "batting":
    ...
    built_results = _build_batting_metric_results(
    ...
    retained = [
        ...
        if not (
            ...
            and item.get("metric_id") in _BATTING_V2_METRIC_IDS
        )
    ]
    ...
    if isinstance(results_payload["meta"], dict):
        results_payload["meta"]["batting_v2_metric_pack"] = _build_metric_pack_summary(
            built_results
        )

    insights = build_batting_v2_findings_insights(built_results)
    findings_payload = results_payload.get("findings")
    if isinstance(findings_payload, dict):
        findings_payload["batting_v2"] = insights
    report_payload = results_payload.get("report")
    if isinstance(report_payload, dict):
        report_payload["batting_v2"] = insights

    return results_payload


def build_batting_v2_findings_insights(
    ...
    for metric in metric_results:
        label = metric.metric_id.replace("batting_", "").replace("_", " ")
        if (
...
def _build_batting_metric_results(
    *,
    ...
    setup_stance = _single_phase_metric(
        repetitions=repetitions,
        phase_lookup=phase_lookup,
        frames=frames,
        phase_names=("setup",),
    ...
    setup_head_offset = _single_phase_metric(
        repetitions=repetitions,
        phase_lookup=phase_lookup,
        frames=frames,
        phase_names=("setup",),
    ...
    trigger_disp = _setup_trigger_displacement_metric(
        repetitions=repetitions,
        phase_lookup=phase_lookup,
        frames=frames,
    )
    downswing_head_stability = _single_phase_metric(
        repetitions=repetitions,
        phase_lookup=phase_lookup,
        frames=frames,
        phase_names=("downswing",),
        measure_fn=_measure_head_stability_score,
    )
    contact_proxy_knee = _single_phase_metric(
        repetitions=repetitions,
        phase_lookup=phase_lookup,
        frames=frames,
        phase_names=("contact_proxy_window", "contact"),
        measure_fn=_measure_front_knee_angle_proxy,
    )
    follow_through_balance = _single_phase_metric(
        repetitions=repetitions,
        phase_lookup=phase_lookup,
        frames=frames,
    ...
    return [
        ...
        _build_metric_result(
            ...
            valid_range=(0.0, 2.0),
            classification_fn=lambda value: (
                "STRONG" if value <= 0.18 else "DEVELOPING" if value <= 0.3 else "NEEDS_ATTENTION"
            ),
            limitations=[
        ...
        _build_metric_result(
            ...
            phase="downswing",
            unit="score",
            samples=downswing_head_stability,
            sample_fps=sample_fps,
            source_video_fps=source_video_fps,
            camera_view=camera_view,
            capture_profile=capture,
        ...
        _build_metric_result(
            ...
            classification_fn=lambda value: (
                "STRONG"
                if 130.0 <= value <= 175.0
                else "DEVELOPING"
                if 110.0 <= value <= 185.0
                else "NEEDS_ATTENTION"
            ),
            limitations=[
        ...
        _build_metric_result(
            ...
            unit="ratio",
            samples=follow_through_balance,
            sample_fps=sample_fps,
            source_video_fps=source_video_fps,
            camera_view=camera_view,
            capture_profile=capture,
...
def _aggregate_stats(
    ...
    payload["spread"] = round(pstdev(values), 4) if len(values) > 1 else 0.0
    return payload
...
def _setup_trigger_displacement_metric(
    *,
    repetitions: list[RepetitionActionRecordV2],
    phase_lookup: dict[tuple[str, str], PhaseRecordV2],
    frames: list[_NormalizedFrame],
    ...
    for repetition in repetitions:
        ...
        value, visibility = _measure_trigger_head_displacement_ratio(setup_window, trigger_window)
        if value is None:
            continue
        samples.append(
            _MetricSample(
                repetition_id=repetition.repetition_id,
                phase_id=trigger_phase.phase_id,
                value=value,
                confidence=_window_confidence(
                    repetition, trigger_phase, visibility, fallback=setup_phase.confidence
                ),
                visibility=visibility,
                start_ts=trigger_phase.start_ts,
                end_ts=trigger_phase.end_ts,
                start_frame=trigger_phase.start_frame,
                end_frame=trigger_phase.end_frame,
            )
        )
    return samples
...
def _measure_head_base_offset_ratio(window: list[_NormalizedFrame]) -> tuple[float | None, float]:
    ...
    for frame in window:
        ...
        if not (nose and left_ankle and right_ankle):
            continue
        stance = _distance(left_ankle, right_ankle)
        if stance <= 0.001:
            continue
        base_mid_x = (left_ankle[0] + right_ankle[0]) / 2.0
        values.append(abs(nose[0] - base_mid_x) / stance)
    return (_mean_or_none(values), _visibility(window, values))


def _measure_trigger_head_displacement_ratio(
    setup_window: list[_NormalizedFrame], trigger_window: list[_NormalizedFrame]
) -> tuple[float | None, float]:
    setup_nose = [
        point
        for frame in setup_window
        if frame.detected
        for point in [frame.keypoints.get("nose")]
        if point is not None
    ]
    trigger_nose = [
        point
        for frame in trigger_window
        if frame.detected
        for point in [frame.keypoints.get("nose")]
        if point is not None
    ]
    if not setup_nose or not trigger_nose:
        return (None, 0.0)

    setup_center = _mean_point(setup_nose)
    trigger_center = _mean_point(trigger_nose)
    if setup_center is None or trigger_center is None:
        return (None, 0.0)
    ...
    displacement = _distance(setup_center, trigger_center) / mean(shoulder_widths)
    total = len(setup_window) + len(trigger_window)
    visibility = (len(setup_nose) + len(trigger_nose)) / total if total > 0 else 0.0
    return (displacement, visibility)
...
def _measure_head_stability_score(window: list[_NormalizedFrame]) -> tuple[float | None, float]:
    ...
    if not movement_values:
        return (None, 0.0)
    avg_movement = mean(movement_values)
    score = max(0.0, min(1.0, 1.0 - (avg_movement / 0.35)))
...
def _build_metric_pack_summary(metric_results: list[CoachingMetricResultV2]) -> dict[str, Any]:
    valid_count = sum(
        1
        for metric in metric_results
    ...
    return {
        "enabled": True,
        "version": _BATTING_V2_METRIC_VERSION,
...
def _evidence_refs(samples: list[_MetricSample]) -> list[EvidenceRef]:
    ...
    dedup: dict[tuple[str, str | None], EvidenceRef] = {}
    for ref in refs:
        dedup[(ref.ref_type, ref.ref_id)] = ref
...
def _timestamp_refs(samples: list[_MetricSample]) -> list[TimestampRef]:
    ...
    for sample in samples:
        refs.append(TimestampRef(start_ts=sample.start_ts, end_ts=sample.end_ts))
    return refs
...
def _frame_refs(samples: list[_MetricSample]) -> list[FrameRef]:
    refs: list[FrameRef] = []
    for sample in samples:
        refs.append(FrameRef(start_frame=sample.start_frame, end_frame=sample.end_frame))
    return refs
...
def _mean_point(points: list[tuple[float, float]]) -> tuple[float, float] | None:
    if not points:
        return None
    return (mean([item[0] for item in points]), mean([item[1] for item in points]))
