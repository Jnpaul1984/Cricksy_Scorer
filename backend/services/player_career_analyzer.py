...
class PlayerCareerAnalyzer:
    ...
    def analyze_player_career(
        ...
        # Analyze recent form
        recent_form = PlayerCareerAnalyzer._analyze_recent_form(batting_records, bowling_records)
    ...
    def _analyze_batting(records: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        consistency = statistics.stdev(runs_list) if len(runs_list) > 1 else 0
    ...
    def _determine_specialization(
        ...
        is_allrounder = (
            ...
            and bowling_stats.get("total_wickets", 0)
            >= PlayerCareerAnalyzer.ALLROUNDER_BOWLING_THRESHOLD
        )
    ...
    def _analyze_recent_form(
        ...
        if len(recent_batting) >= 3:
            ...
            second_half_avg = sum(r.get("runs", 0) for r in recent_batting[2:]) / len(
                recent_batting[2:]
            )
    ...
    def _get_best_performances(
        ...
        if bowling_records:
            ...
            performances["best_bowling"] = {  # type: ignore[assignment]
                ...
                "economy": round(
                    best_bowl.get("runs_conceded", 0) / best_bowl.get("overs_bowled", 1), 2
                ),
    ...
    def _generate_career_highlights(
        ...
        if bowling_stats:
            ...
            if bowling_stats.get("maiden_percentage", 0) > 15:
                highlights.append(
                    f"⭐ Strong control bowler ({bowling_stats.get('maiden_percentage', 0):.0f}% maidens)"
                )
    ...
    def _generate_summary(
        ...
        if bowling_stats and bowling_stats.get("total_wickets", 0) > 0:
            ...
            parts.append(
                f"With the ball, {player_name} has {wickets} wickets at an economy of {economy}."
            )

        # Recent form
        trend = recent_form.get("trend", "stable")
        if trend == "improving":
            parts.append("Recent form is improving - showing positive momentum in recent matches.")
        elif trend == "declining":
            parts.append("Recent form has been declining - looking to regain consistency.")
        else:
            parts.append("Playing with steady, consistent performances recently.")
