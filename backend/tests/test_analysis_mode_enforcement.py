...
class TestServiceValidation:
    ...
    async def test_service_accepts_valid_modes(self):
        ...
        for mode in valid_modes:
            ...
            with patch(
                "backend.services.pose_service.extract_pose_keypoints_from_video"
            ) as mock_extract:
...
class TestFindingsCodeFiltering:
    ...
    def test_bowling_mode_allows_only_bowling_codes(self):
        ...
        for finding in findings:
            code = finding["code"]
            assert (
                code in ALLOWED_CODES_BY_MODE["bowling"]
    ...
    def test_wicketkeeping_mode_allows_only_wicketkeeping_codes(self):
        ...
        for finding in findings:
            code = finding["code"]
            assert (
                code in ALLOWED_CODES_BY_MODE["wicketkeeping"]
    ...
    def test_fielding_mode_allows_only_fielding_codes(self):
        ...
        for finding in findings:
            code = finding["code"]
            assert (
                code in ALLOWED_CODES_BY_MODE["fielding"]
...
class TestModeAwareNarratives:
    ...
    def test_head_movement_has_bowling_narrative(self):
        ...
        if head_findings:
            ...
            assert (
                "line and length" in why_it_matters.lower() or "delivery" in why_it_matters.lower()
    ...
    def test_knee_collapse_has_wicketkeeping_warning(self):
        ...
        if knee_findings:
            ...
            if knee_finding["severity"] == "high":
                ...
                has_wk_warning = any("keeping" in cue.lower() for cue in cues)

                # Should NOT have batting warning
                has_batting_warning = any("Suspend intensive batting" in cue for cue in cues)

                assert (
                    not has_batting_warning
...
class TestModeAwareDictionaries:
    ...
    def test_why_it_matters_has_common_codes(self):
        ...
        for code in expected_codes:
            assert (
                code in WHY_IT_MATTERS_BY_MODE
