...
class TestPricingContract:
    ...
    def test_all_individual_plans_have_required_fields(self) -> None:
        ...
        for plan in IndividualPlan:
            ...
            assert details["price_monthly_usd"] is None or isinstance(details["price_monthly_usd"], float)
            assert details["video_storage_bytes"] is None or isinstance(details["video_storage_bytes"], int)
            assert details["max_video_duration_seconds"] is None or isinstance(details["max_video_duration_seconds"], int)
            assert details["ai_reports_per_month"] is None or isinstance(details["ai_reports_per_month"], int)
    ...
    def test_org_pro_has_unlimited_storage(self) -> None:
        ...
        assert org_details["max_video_duration_seconds"] is None, "Org Pro should have unlimited duration"
    ...
    def test_free_plan_has_no_video_features(self) -> None:
        ...
        assert features.get("video_upload_enabled", False) is False, "Free plan should not allow video uploads"
        assert features.get("video_sessions_enabled", False) is False, "Free plan should not have video sessions"
    ...
    def test_coach_pro_plus_video_quota(self) -> None:
        ...
        assert details["video_storage_bytes"] == expected_bytes, "Coach Pro Plus should have 25GB storage"
        
        # 2 hours in seconds
        expected_duration = 7200
        assert details["max_video_duration_seconds"] == expected_duration, "Coach Pro Plus should allow 2 hour videos"
    ...
    def test_pricing_snapshot(self) -> None:
        ...
        assert INDIVIDUAL_PRICES[IndividualPlan.COACH_LIVE_AI_ADVANCED]["monthly_usd"] == Decimal("99.99")
