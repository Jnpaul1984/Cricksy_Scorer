...
async def test_pdf_export_route_is_registered():
    ...
    export_pdf_routes = [r for r in routes_info if "export-pdf" in r["path"]]
    ...
    if not matching_route:
        all_coach_routes = [r for r in routes_info if "/api/coaches/plus/" in r["path"]]

        error_msg = f"\n{'='*80}\n"
        error_msg += "ROUTE REGISTRATION FAILURE\n"
        error_msg += f"{'='*80}\n\n"
        error_msg += "Expected route NOT FOUND:\n"
