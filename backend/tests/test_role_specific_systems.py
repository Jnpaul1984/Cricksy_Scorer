...
BATTING_SPECIFIC_PHRASES = [
    ...
    "batsman",
    "batter",  # batting roles
    "backswing",
    "downswing",
    "backlift",
    "trigger",  # batting mechanics
    "cover drive",
    "pull shot",
    "cut shot",
    "on drive",
    "off drive",  # batting shots
    "playing a shot",
    "executing a shot",  # batting actions
]
...
def test_no_batting_terminology_in_non_batting_modes(mode):
    ...
    found_batting_phrases = [
        phrase for phrase in BATTING_SPECIFIC_PHRASES if phrase in combined_text
    ]
...
@pytest.mark.parametrize(
    "mode,drill_code,expected_count",
    [
...
def test_drills_come_from_correct_mode_registry(mode, drill_code, expected_count):
    ...
    if mode == "bowling":
        ...
        assert any(
            term in combined for term in ["bowl", "delivery", "action", "pace", "seam"]
        ...
        has_keeper_terms = any(
            term in combined for term in ["keep", "glove", "crouch", "take", "keeper"]
        )
        has_generic_terms = any(
            term in combined for term in ["balance", "footwork", "position", "movement"]
        )
        assert (
            has_keeper_terms or has_generic_terms
...
@pytest.mark.parametrize(
    "metric,bowling,batting,wicketkeeping,fielding",
    [
...
@pytest.mark.parametrize(
    "metric,bowling_strict,wicketkeeping_relaxed",
    [
...
def test_bowling_strictest_wicketkeeping_most_relaxed(
    metric, bowling_strict, wicketkeeping_relaxed
):
...
def test_confidence_fields_in_results():
    ...
    assert isinstance(
        result["confidence_factors"], list
    ), "confidence_factors should be list (not dict)"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            0.50,
        ),  # ABOVE wicketkeeping threshold (0.58) = no finding
    ],
)
def test_thresholds_affect_finding_generation(mode, metric, score):
    ...
    if mode == "wicketkeeping":
        ...
        head_findings = [f for f in result["findings"] if "head" in f["code"].lower()]
        assert len(head_findings) > 0, f"{mode} should generate head finding for score {score}"


def test_check_functions_use_mode_thresholds():
    """Test that _check_* functions use mode-specific thresholds."""
    metrics = {"head_stability_score": {"score": 0.62, "num_frames": 100}}
...
def test_check_functions_use_mode_drills():
    ...
    assert (
        bowling_drills != fielding_drills
    ...
    assert any(
        term in bowling_text for term in ["bowl", "delivery", "action"]
...
def test_all_modes_have_drills_and_thresholds(mode):
    ...
    for threshold_key in required_thresholds:
        assert threshold_key in mode_thresholds, f"{mode} missing threshold: {threshold_key}"
