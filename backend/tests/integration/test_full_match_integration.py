...
def test_complete_match_no_wickets(game_helper, assert_helper):
    ...
    for i in range(12):
        ...
        response = game_helper.post_delivery(
            batsman_id=batsman, bowler_id=current_bowler, runs_scored=1
        )
    ...
    game_helper.start_next_innings(
        striker_id=striker2, non_striker_id=non_striker2, opening_bowler_id=bowler3
    )
...
def test_complete_match_with_wickets(game_helper, assert_helper):
    ...
    for i in range(2):
        response = game_helper.post_delivery(
            batsman_id=new_batsman, bowler_id=bowler1, runs_scored=1
        )
        assert response.status_code == 200
...
def test_complete_20_over_match(game_helper, assert_helper):
    ...
    for i in range(120):
        ...
        response = game_helper.post_delivery(
            batsman_id=batsman, bowler_id=current_bowler, runs_scored=runs
        )
        assert response.status_code == 200
...
def test_match_with_wickets_and_extras(game_helper, assert_helper):
    ...
    # Post wide
    game_helper.post_delivery(striker, bowler, runs_scored=1, extra_type="wd")

    # Post wicket
    game_helper.post_delivery(
        striker, bowler, runs_scored=0, is_wicket=True, dismissal_type="bowled"
    )
...
def test_innings_transition(game_helper, assert_helper):
    ...
    game_helper.start_next_innings(
        striker_id=striker2, non_striker_id=non_striker2, opening_bowler_id=bowler2
    )
