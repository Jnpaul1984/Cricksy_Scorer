...
class GameHelper:
    ...
    def post_delivery(
        ...
        payload = {"bowler_id": bowler_id, "runs_scored": runs_scored}
    ...
    def select_next_batsman(self, batter_id: str) -> dict[str, Any]:
        ...
        response = self.client.post(
            f"/games/{self.game_id}/next-batter", json={"batter_id": batter_id}
        )
        assert response.status_code == 200, f"Failed to select batsman: {response.text}"
        return response.json()

    def start_next_innings(
        self, striker_id: str, non_striker_id: str, opening_bowler_id: str
    ) -> dict[str, Any]:
...
class AssertionHelper:
    ...
    def assert_pending_batsman(snapshot: dict, expected: bool):
        ...
        assert actual == expected, f"Expected needs_new_batter={expected}, got {actual}"
