...
def engineer_features(df: pd.DataFrame, match_format: str) -> pd.DataFrame:
    ...
    def calc_rolling_features(group):
        ...
        group['run_rate_last_3'] = group['total_runs'].diff(periods=min(3, len(group))).fillna(0) / 3
        ...
        group['dot_ratio_last_6'] = (group['total_runs'].diff().fillna(0) == 0).rolling(6, min_periods=1).mean()
        group['boundary_density_last_3'] = (group['total_runs'].diff().fillna(0) >= 4).rolling(3, min_periods=1).mean()
...
def create_target_variable(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['target'] = (df['total_runs'] / df['completed_overs'].replace(0, 0.1) * 
                    (df['completed_overs'] + df['balls_remaining']/6) > df['final_score']).astype(int)
...
def train_win_predictor(match_format: Literal['t20', 'odi']) -> None:
    ...
    print(f"Target distribution:\n  Loss (0): {(y==0).sum():,} ({(y==0).mean()*100:.1f}%)\n  Win (1): {(y==1).sum():,} ({(y==1).mean()*100:.1f}%)")
