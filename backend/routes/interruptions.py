...
def _last_open_index(
    history: List[Dict[str, Any]], kind: Optional[Kind]
) -> Optional[int]:
...
def _start_core(
    game: Game, kind: Kind, note: Optional[str], at_utc: Optional[dt.datetime]
) -> List[Dict[str, Any]]:
...
def _stop_core(
    game: Game, kind: Optional[Kind], at_utc: Optional[dt.datetime]
) -> List[Dict[str, Any]]:
    history: List[Dict[str, Any]] = _normalize_history(list(game.interruptions or []))
