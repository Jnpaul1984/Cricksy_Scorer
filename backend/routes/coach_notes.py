...
from backend.sql_app.models import CoachNote, CoachNoteSeverity, Player, RoleEnum, User, VideoSession
...
@router.post("/players/{player_id}/notes", response_model=CoachNoteRead, status_code=status.HTTP_201_CREATED)
async def create_coach_note(
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f"Invalid severity. Must be one of: info, improvement, critical",
        )
...
async def list_player_notes(
    player_id: int,
    current_user: Annotated[User, Depends(security.get_current_active_user)],
    severity: str | None = Query(None, description="Filter by severity: info, improvement, critical"),
    ...
    return [
        CoachNoteRead(
            ...
            video_session_id=note.video_session_id,
            video_session_title=sessions_dict.get(note.video_session_id) if note.video_session_id else None,
            moment_marker_id=note.moment_marker_id,
            note_text=note.note_text,
            tags=note.tags,
            severity=note.severity.value,
