...
def upgrade():
    ...
    op.execute("""
        DO $$ BEGIN
            CREATE TYPE video_moment_type AS ENUM ('setup', 'catch', 'throw', 'dive', 'stumping', 'other');
        EXCEPTION
    ...
    op.create_table(
        ...
        sa.Column('timestamp_ms', sa.BigInteger(), nullable=False, comment='Timestamp in milliseconds from video start'),
        sa.Column('moment_type', postgresql.ENUM('setup', 'catch', 'throw', 'dive', 'stumping', 'other', name='video_moment_type', create_type=False), nullable=False),
        sa.Column('description', sa.Text(), nullable=True),
        sa.Column('created_by', sa.String(), nullable=False, comment='Coach user_id who created marker'),
    ...
    # Create indexes
    op.create_index('ix_video_moment_markers_video_session_id', 'video_moment_markers', ['video_session_id'], unique=False)
    op.create_index('ix_video_moment_markers_moment_type', 'video_moment_markers', ['moment_type'], unique=False)
    op.create_index('ix_video_moment_markers_timestamp_ms', 'video_moment_markers', ['timestamp_ms'], unique=False)
    ...
    op.create_index('ix_coach_notes_moment_marker_id', 'coach_notes', ['moment_marker_id'], unique=False)
