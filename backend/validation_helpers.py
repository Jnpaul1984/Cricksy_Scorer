...
def validate_player_in_team(player_id: str, team: dict, team_name: str) -> None:
    ...
    if str(player_id) not in team_players:
        raise HTTPException(
            status_code=422, detail=f"Player {player_id} is not in team {team_name}"
        )


def validate_batsman_in_batting_team(
    batsman_id: str, team_a: dict, team_b: dict, batting_team_name: str
) -> None:
    """
...
def validate_bowler_in_bowling_team(
    bowler_id: str, team_a: dict, team_b: dict, bowling_team_name: str
) -> None:
    """
...
def validate_no_same_player_batting_and_bowling(
    striker_id: str | None, non_striker_id: str | None, bowler_id: str | None
) -> None:
    """
...
def validate_dismissal_type(is_wicket: bool, dismissal_type: str | None) -> None:
    """
    ...
    if not dismissal_type:
        raise HTTPException(
            status_code=422, detail="Dismissal type is required when is_wicket is true"
        )
