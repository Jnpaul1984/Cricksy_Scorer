        ...
        if line.startswith('- id:'):
            ...
            # List items (  - )
                ...
                if not in_list:
                    ...
                    # Detect list key from context (acceptance_criteria, verification_commands, etc.)
                    # Look back in current_item
                    for k in ['acceptance_criteria', 'verification_commands', 'manual_checks', 'files_touched']:
...
def cmd_status(week: Optional[int] = None):
...
def cmd_done(item_id: str, agent_name: str, files_str: str):
    ...
    if item_id not in verifications:
        ...
        print(f"   You must run verification commands AFTER code changes.")
...
def cmd_verify(item_id: str):
    ...
    if all_passed:
        print("\n" + "="*60)
        print(f"✅ All verification commands passed!")
        ...
        print("\n" + "="*60)
        print(f"❌ Some commands failed. Fix and re-run.")
        sys.exit(1)
